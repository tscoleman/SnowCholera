---
title: "John Snow Project - Reading Snow data, adjusting, and building regression dataframes"
author: "[Thomas Coleman](http://www.hilerun.org/econ)"
output: html_notebook
---
# New (functional) version for Reading in data from 1855 Table VII, XII; Table 1856 Tables I & II, VI
...
+ Reading in data from Snow's and Simon's tables
+ Adjusting subdistrict by-supplier data:
  1) Zero out Lambeth population in S&V-only subdistricts (cf Snow 1856 p 245)
  2) Increase subdistrict by-supplier population so that the sum of subdistricts within each district match the district totals reported in Simon Table II. (In subdistrict reporting in Simon Table III, houses in streets with no deaths were not allocated by supplier, and the totals for this unassigned population is reported by supplier for each district in Simon Table III)
  3) Decrease subdistrict population when the by-supplier estimates are larger than Simon' 1854 estimates 

+ The following data tables should be read in by the calling script / worksheet
  + From Snow 1855 "On the mode of communication of cholera"
  ...
  + From John Snow 1856, "Cholera and the water supply in the south district of London in 1854". These data were copied from the 1936 book "Snow on cholera, being a reprint of two papers" edited by Frost
  ...
  + From John Simon "Report on the last two cholera-epidemics of London, as affected by the consumption of impure water"
    + Population data from Simon's Table III:
    + tableSimonIII <- read.csv(file="Simon1856_TableIII_pop.csv",header=TRUE, sep=",", skip=5,comment.char="#")
    + Population by-supplier for "Houses supplied in streets where no death occurred" which reports by *districts* and must be allocated back to subdistricts
    + tableSimonIII_StNoDeath <- read.csv(file="Simon1856_TableIII_StNoDeath.csv",header=TRUE, sep=",", skip=6,comment.char="#")

+ Functions for creating the regression dataframes necessary (plus overdisp_fun)
  + overdisp_fun(model,type="deviance")
    + returns xoverdisp,xPseudoRsq,lnRateRsq,RateRsq,RateSD,lnRateSD,BIC
  + regdata for the DiD comparison of 1855 Table XII data
    + cr_regdata(tableI_1856,tablevii,tableviii,tablexii,tableV_1856,tableVI_1856,tableSimonIII,tableSimonIII_StNoDeath,TableGROElevation,population_adj=3,population_1851=0,tableVIdeaths=0)
  + Regression data for DiD separating out randomized by-supplier for 1st 7weeks of 1854
    + cr_regdata_mixedDiD7wks(tableI_1856,tablevii,tableviii,tablexii,tableV_1856,tableVI_1856,tableSimonIII,tableSimonIII_StNoDeath,TableGROElevation,population_adj=3,population_1851=0,tableVIdeaths=0)
    + returns list(regdata_misdDiD7wks, regdata_mixedDiD7wks_xOther)
  + Regression data for 1855 by-supplier data from Table VIII (1st 7 weeks)
    + cr_regdata1855VIII(tableI_1856,tablevii,tableviii,tablexii,tableV_1856,tableVI_1856,tableSimonIII,tableSimonIII_StNoDeath,TableGROElevation,population_adj=3,population_1851=0,tableVIdeaths=0)
    + returns list(regdata1855VIIIboth, regdata1855VIIIjoint)
  + Regression data for combined DiD & Randomization
    + cr_regdata_mixedDiD4wks(tableI_1856,tablevii,tableviii,tablexii,tableV_1856,tableVI_1856,tableSimonIII,tableSimonIII_StNoDeath,TableGROElevation,population_adj=3,population_1851=0,tableVIdeaths=0)
    + returns list(regdata_mixedDiD1st4wks,regdata_mixedDiD1st4wks_xOther)
  + Creates matrix comparing different sums (for "first 12" and "next 16") of by-supplier population for 1854 & 1851
    + cr_popcom(tableI_1856,tablevii,tableviii,tablexii,tableV_1856,tableVI_1856,tableSimonIII,tableSimonIII_StNoDeath)
    + returns dataframe 4x6: 4 rows for population_adj = 0, 1, 2, 3; 3 cols for S&V, Lambeth, Combined (1854); 3 cols for S&V, Lambeth, Combined (1851)
  + Create combined (stacked) data for all the regressions
    + cr_data_combined(tableI_1856,tablevii,tableviii,tablexii,tableV_1856,tableVI_1856,tableSimonIII,tableSimonIII_StNoDeath,TableGROElevation,population_adj=3,population_1851=0,tableVIdeaths=0)
    + Used within the above functions which then extract out the appropriate data
...
# See also "Causality in the Time of Cholera: John Snow as a Prototype 
...
```{r message=FALSE, results='hide'}
# Function for calculating statistics associated with overdispersion


# Modified from excellent site http://bbolker.github.io/mixedmodels-misc/glmmFAQ.html
...
overdisp_fun <- function(model,type="deviance",nulldeviance=Inf) {
  # Works for type="pearson" or "deviance"
  # I need to pass in the null deviance for linear models because the linear models are run 
  #   without a constant. This null deviance should be the residual deviance from a model run with
  #   just population (link="identity") and no constant
    xn <- nobs(model)
    ...
    xlink <- model$family[2]  # will be "identity" for a linear model
  # Now calculate the pseudo-Rsq and the rate Rsq
    if (class(model)[1] == "glmerMod") {
      ...
      if (xlink == "identity") {  # if linear model then alternate calculation for pseudo-Rsq
      	xPseudoRsq <- 1 - (model$deviance/nulldeviance)
      }
    }
    ...
    xoffset <- x1[,x2[2]]
    xpred <- predict(model)
    xrate_act <- xlcount / exp(xoffset)
    xrate_pred <- exp(xpred) / exp(xoffset)
    if (xlink == "identity") {  # if linear model then alternate calculation for rates and log rates
      xpop <- x1[,2]     # for linear model, population will be in 2nd positio
      xrate_act <- xlcount / xpop
      xrate_pred <- xpred / xpop
    }
    xRsq <- (cor(xrate_pred,xrate_act))^2
    xAdjRsq <- 1 - (1-xRsq)*(xn-1)/(rdf-1)
    xsdrate <- 10000 * sd(xrate_act)
    xlrate_act <- log(xrate_act)
    xlrate_pred <- log(xrate_pred)
    xlnRsq <- (cor(xlrate_pred,xlrate_act))^2
    xsdlrate <- sd(xlrate_act)
    #x1 <- list(xoverdisp,xpseudoRsq)
    xoverdisp <- append(xoverdisp,c(PseudoRsq=xPseudoRsq,lnRateRsq=xlnRsq,RateRsq=xRsq,AdjRateRsq=xAdjRsq,RateSD=xsdrate,lnRateSD=xsdlrate,BIC=BIC(model),AIC=AIC(model),logLik=logLik(model)))
    xoverdisp
}


overdisp_fun_old <- function(model,type="deviance") {
  # Works for type="pearson" or "deviance"
    rdf <- df.residual(model)
    rp <- residuals(model,type=type)
    resid.chisq <- sum(rp^2)   # This is the "Residual Deviance"
  ...
  # Now calculate the pseudo-Rsq and the rate Rsq
    ...
    xsdlrate <- sd(xlrate_act)
    #x1 <- list(xoverdisp,xpseudoRsq)
    xoverdisp <- append(xoverdisp,c(PseudoRsq=xPseudoRsq,lnRateRsq=xlnRsq,RateRsq=xRsq,RateSD=xsdrate,lnRateSD=xsdlrate,BIC=BIC(model),AIC=AIC(model),logLik=logLik(model)))
    xoverdisp
}
...
cr_data_internal <- function(tableI_1856,tablevii,tableviii,tablexii,tableV_1856,tableVI_1856,tableSimonIII,tableSimonIII_StNoDeath,TableGROElevation,population_adj=3,population_1851=0,tableVIdeaths=0) {
  # returns list(xdata_combined,xdata_combined_1st4)
  ...
  # Check if we want to use deaths from the 1856 Table VI
  if(tableVIdeaths == 1) {
    x1 <- tableVI_1856$deaths_1854[match(tablexii$subDistrict,tableVI_1856$subDistrict)]  # deaths 1854 from Table VI in order of tablexii
    x2 <- x1[!is.na(x1)]
    xx2$deaths1854[xx2$subDistrict != "Sydenham"] <- x2 # 1856 Table VI does not have Sydenham
  }
  # Join the elevation
  xx2 <- left_join(xx2,tableGROElevation[,c("subDistrict","elevation")],by='subDistrict')
  ...
  xx4$perc_southwark_0 <- xpop_southwark_0 / xpop
  xx4$perc_lambeth_0 <- xpop_lambeth_0 / xpop
  ...
  xx4$perc_southwark_1 <- xpop_southwark_1 / xpop
  xx4$perc_lambeth_1 <- xpop_lambeth_1 / xpop
  ...
  xx4$perc_southwark_2 <- xpop_southwark_2 / xpop
  xx4$perc_lambeth_2 <- xpop_lambeth_2 / xpop
  ...
  xpop_combined_3 <- pmin(xpop,xpop_combined_2)
  ...
  xx4$perc_southwark_3 <- (xpop_southwark_3/xpop) 
  xx4$perc_lambeth_3 <- (xpop_lambeth_3/xpop)
  # Increase overall as appropriate. NB: THIS MAY BE DANGEROUS
  xpop_combined_4 <- xpop_combined_2
  xpop_total <- pmax(xpop,xpop_combined_2)
  ...
  xx4$perc_combined_adj <- (xpop_combined_3 / xpop)
  ...
  if(population_adj == 0) {
    ...
    xx4$perc_combined_adj <- (xpop_combined_0 / xpop)
  ...
  if(population_adj == 1) {
    ...
    xx4$perc_combined_adj <- (xpop_combined_1 / xpop)
  ...
  if(population_adj == 2) {
    ...
    xx4$perc_combined_adj <- (xpop_combined_2 / xpop)
  ...
  xx4$pop_other_adj <- xpop - xx4$pop_combined_adj
  ...
  # Put in percentage by supplier. For 1854 this is the calculated 1854 percentage, for 1849 use the same value  
  ...
  # Also put in the by-supplier population, but it will only be for 1854 (put in NA for 1849)
  x1849$pop_other <- NA  # xxx
  x1854$pop_other <- x1$pop_other_adj
  x1849$pop_lambeth <- NA
  x1854$pop_lambeth <- x1$pop_lambeth_adj
  x1849$pop_lambeth54 <- NA
  x1854$pop_lambeth54 <- x1$pop_lambeth_adj
  x1849$pop_southwark <- NA
  x1854$pop_southwark <- x1$pop_southwark_adj
  ...
  # elevation
  x1849$elevation <- x1$elevation
  x1854$elevation <- x1$elevation
  ...
  x1854VII_other <- x1854VII_other[!is.na(x1854VII_other$population),]
  ...
  list(xdata_combined,xdata_combined_1st4)
}


cr_data_combined <- function(tableI_1856,tablevii,tableviii,tablexii,tableV_1856,tableVI_1856,tableSimonIII,tableSimonIII_StNoDeath,TableGROElevation,population_adj=3,population_1851=0,tableVIdeaths=0) {

  x1 <- cr_data_internal(tableI_1856,tablevii,tableviii,tablexii,tableV_1856,tableVI_1856,tableSimonIII,tableSimonIII_StNoDeath,TableGROElevation,population_adj,population_1851,tableVIdeaths)
  xdata_combined <- x1[[1]]
  xdata_combined
}


cr_data_combined_1st4 <- function(tableI_1856,tablevii,tableviii,tablexii,tableV_1856,tableVI_1856,tableSimonIII,tableSimonIII_StNoDeath,TableGROElevation,population_adj=3,population_1851=0,tableVIdeaths=0) {
  
    x1 <- cr_data_internal(tableI_1856,tablevii,tableviii,tablexii,tableV_1856,tableVI_1856,tableSimonIII,tableSimonIII_StNoDeath,TableGROElevation,population_adj,population_1851,tableVIdeaths)
  xdata_combined_1st4 <- x1[[2]]
  xdata_combined_1st4
}
...
```{r}
cr_regdata <- function(tableI_1856,tablevii,tableviii,tablexii,tableV_1856,tableVI_1856,tableSimonIII,tableSimonIII_StNoDeath,TableGROElevation,population_adj=0,population_1851=1,tableVIdeaths=0,excludeLambethOnly=1) {
  # Create regression data for simple DiD using 1855 Table XII (and population 1851 from Table VIII)
  xdata_combined <- cr_data_combined(tableI_1856,tablevii,tableviii,tablexii,tableV_1856,tableVI_1856,tableSimonIII,tableSimonIII_StNoDeath,TableGROElevation,population_adj,population_1851,tableVIdeaths)
  x1 <- xdata_combined
  # Default is to Select the "First 12" and "Next 16" subdistricts - i.e. exclude the "Lambeth Co only" subdistricts
  if(excludeLambethOnly == 1) {
...
cr_regdata_mixedDiD7wks <- function(tableI_1856,tablevii,tableviii,tablexii,tableV_1856,tableVI_1856,tableSimonIII,tableSimonIII_StNoDeath,TableGROElevation,population_adj,population_1851,tableVIdeaths) {
  # Regression data for DiD separating out randomized by-supplier for 1st 7weeks of 1854
  xdata_combined <- cr_data_combined(tableI_1856,tablevii,tableviii,tablexii,tableV_1856,tableVI_1856,tableSimonIII,tableSimonIII_StNoDeath,TableGROElevation,population_adj,population_1851,tableVIdeaths)
  x1 <- subset(xdata_combined,supplier == "SouthwarkVauxhall" | supplier == "SouthwarkVauxhall_Lambeth")
  # regdata is for DiD with 1849 & 1854 combined data only
  ...
  list(regdata_misdDiD7wks, regdata_mixedDiD7wks_xOther)
}



cr_regdata1855VIII <- function(tableI_1856,tablevii,tableviii,tablexii,tableV_1856,tableVI_1856,tableSimonIII,tableSimonIII_StNoDeath,TableGROElevation,population_adj,population_1851,tableVIdeaths) {
  # Regression data for 1855 by-supplier data from Table VIII (1st 7 weeks)
  xdata_combined <- cr_data_combined(tableI_1856,tablevii,tableviii,tablexii,tableV_1856,tableVI_1856,tableSimonIII,tableSimonIII_StNoDeath,TableGROElevation,population_adj,population_1851,tableVIdeaths)
  x1 <- subset(xdata_combined,supplier == "SouthwarkVauxhall" | supplier == "SouthwarkVauxhall_Lambeth")
  ...
  list(regdata1855VIIIboth, regdata1855VIIIjoint)
}

cr_regdata_mixedDiD4wks <- function(tableI_1856,tablevii,tableviii,tablexii,tableV_1856,tableVI_1856,tableSimonIII,tableSimonIII_StNoDeath,TableGROElevation,population_adj,population_1851,tableVIdeaths) {
  # Regression data for combined DiD & Randomization
  # Select the "First 12" and "Next 16" subdistricts - i.e. exclude the "Lambeth Co only" subdistricts
  xdata_combined_1st4 <- cr_data_combined_1st4(tableI_1856,tablevii,tableviii,tablexii,tableV_1856,tableVI_1856,tableSimonIII,tableSimonIII_StNoDeath,TableGROElevation,population_adj,population_1851,tableVIdeaths)
  ...
  list(regdata_mixedDiD1st4wks,regdata_mixedDiD1st4wks_xOther)
}

cr_data_sidebyside <- function(tableI_1856,tablevii,tableviii,tablexii,tableV_1856,tableVI_1856,tableSimonIII,tableSimonIII_StNoDeath,TableGROElevation,population_adj=0,population_1851=1,tableVIdeaths=0) {
    xdata_combined <- cr_data_combined(tableI_1856,tablevii,tableviii,tablexii,tableV_1856,tableVI_1856,tableSimonIII,tableSimonIII_StNoDeath,TableGROElevation,population_adj,population_1851,tableVIdeaths)
  xoutput <- xdata_combined[xdata_combined$year_split1st7 == "1854" ,c("district","subDistrict","pop1851","supplier","lambethdegree","pop_southwark","pop_lambeth","pop_other","perc_southwark","perc_lambeth","perc_other","pop_per_house")]
  xoutput$deaths1849 <- xdata_combined[xdata_combined$year_split1st7 == "1849",c("deaths")]
  xoutput$rate1849 <- xdata_combined[xdata_combined$year_split1st7 == "1849",c("rate")]
  xoutput$pop1849 <- xdata_combined[xdata_combined$year_split1st7 == "1849",c("population")]
  xoutput$deaths1854 <- xdata_combined[xdata_combined$year_split1st7 == "1854",c("deaths")]
  xoutput$rate1854 <- xdata_combined[xdata_combined$year_split1st7 == "1854",c("rate")]
  xoutput$pop1854 <- xdata_combined[xdata_combined$year_split1st7 == "1854",c("population")]
  ...
  #yyy
#  xoutput <- xoutput[,  c("pop1851","deathsOverall","deathsSouthwark","deathsLambeth","deathsPump","deathThames","deaths1849","deaths1854","perc_southwark","perc_lambeth","perc_other")]
  xoutput
}
...
+ Adjustment (1), zeroing Lambeth population in Vauxhall-only subdistricts
+ Adjustment (2), upwards so subdistrict sums (from Table VI) match district reports (Table V)
+ Adjustment (3), adjusting down by-supplier population if combined greater than 1854

```{r, include=FALSE, eval=FALSE}


cr_popcom <- function(tableI_1856,tablevii,tableviii,tablexii,tableV_1856,tableVI_1856,tableSimonIII,tableSimonIII_StNoDeath) {
  # Creates matrix comparing different sums (for "first 12" and "next 16") of by-supplier population for 1854
  # Brute force, creating multiple version of "xdata_combined" for different combinations of "population_adj" and
  #   "population_1851"
  
  # tableI_1856,tablevii,tableviii,tableV_1856,tableSimonIII,tableSimonIII_StNoDeath,population_adj=3,population_1851=0)
  
  xpopcom <- matrix(0,nrow=4,ncol=6)
  rownames(xpopcom) <- c("Adj0","Adj1","Adj2","Adj3")
  colnames(xpopcom) <- c("S&VCustomers","Lambeth","Combined","1851S&V","1851Lambeth","1851Combined")
  ...
  for (i in 0:3) {
    population_adj <- i
    xdata_combined <- cr_data_combined(tableI_1856,tablevii,tableviii,tablexii,tableV_1856,tableVI_1856,tableSimonIII,tableSimonIII_StNoDeath,TableGROElevation,population_adj,population_1851,tableVIdeaths)
    xpopcom[i+1,2] <- sum(xdata_combined$pop_lambeth[xdata_combined$year_split1st7 == "1854" & xdata_combined$supplier != "Lambeth"])
    xpopcom[i+1,1] <- sum(xdata_combined$pop_southwark[xdata_combined$year_split1st7 == "1854" & xdata_combined$supplier != "Lambeth"])
  }
  population_1851 <- 1
  for (i in 0:3) {
    population_adj <- i
    xdata_combined <- cr_data_combined(tableI_1856,tablevii,tableviii,tablexii,tableV_1856,tableVI_1856,tableSimonIII,tableSimonIII_StNoDeath,TableGROElevation,population_adj,population_1851,tableVIdeaths)
    xpopcom[i+1,5] <- sum(xdata_combined$pop_lambeth[xdata_combined$year_split1st7 == "1854" & xdata_combined$supplier != "Lambeth"])
    xpopcom[i+1,4] <- sum(xdata_combined$pop_southwark[xdata_combined$year_split1st7 == "1854" & xdata_combined$supplier != "Lambeth"])
  }

  xpopcom[,3] <- xpopcom[,1] + xpopcom[,2]
  xpopcom[,6] <- xpopcom[,4] + xpopcom[,5]
  xpopcom <- as.data.frame(xpopcom)
